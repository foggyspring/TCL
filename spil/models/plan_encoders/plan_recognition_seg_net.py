...
class PlanRecognitionTransformersNetwork(nn.Module):
    def __init__(
        ...
        self.padding = False
        self.dist = dist
        self.hidden_size = fc_hidden_size # 4096
    ...
    def forward(self, perceptual_emb: torch.Tensor, perceptual_emb_seg: torch.Tensor = None) -> Tuple[State, torch.Tensor, torch.Tensor]:
        ...
        my_state = self.fc_state(x) # output of the transformer, output size is plan_features
        state = self.dist.forward_dist(my_state) # transfer to continuous or discrete state
        
        # perceptual_emb for segmented inputs
        # perceptual_emb_seg = None
        if perceptual_emb_seg is not None:
            batch_size, seq_len_seg = perceptual_emb_seg.shape[0], perceptual_emb_seg.shape[1]
            perceptual_emb_seg = (
                torch.cat([perceptual_emb_seg, torch.zeros((batch_size, seq_len_seg, self.pad)).to(perceptual_emb_seg.device)], dim=-1)
                if self.padding
                else perceptual_emb_seg
            )
            if self.position_embedding:
                position_ids = torch.arange(seq_len_seg, dtype=torch.long, device=perceptual_emb_seg.device).unsqueeze(0)
                position_embeddings = self.position_embeddings(position_ids)
                x_seg = perceptual_emb_seg + position_embeddings
                x_seg = x_seg.permute(1, 0, 2)
            else:
                # padd the perceptual embeddig
                x_seg = self.positional_encoder(perceptual_emb_seg.permute(1, 0, 2))  # [s, b, emb]
            if self.positional_normalize:
                x_seg = self.layernorm(x_seg)
                
            x_seg = self.dropout(x_seg)
            x_seg = self.transformer_encoder(x_seg) # the first dimension is the sequence length, required by the transformer
            x_seg = self.fc(x_seg.permute(1, 0, 2)) # [b, s, emb]
            
            x_seg = torch.mean(x_seg, dim=1)  # gather all the sequence info, [b, emb] global average pooling
            x_seg = torch.nan_to_num(x_seg, posinf=torch.finfo(torch.float16).max, neginf=torch.finfo(torch.float16).min)
        else:
            x_seg = x
            
        return state, x, x_seg
