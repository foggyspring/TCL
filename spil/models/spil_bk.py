...
class Spil(pl.LightningModule):
    ...
    def __init__(
        ...
        use_temporal_contrastive_learning: bool,
        ...
        self.use_temporal_contrastive_learning = use_temporal_contrastive_learning
    ...
    def training_step(self, batch: Dict[str, Dict], batch_idx: int) -> torch.Tensor:  # type: ignore
        ...
        for self.modality_scope, dataset_batch in batch.items():
            ...
            if not self.use_temporal_contrastive_learning:
                perceptual_emb = self.perceptual_encoder(
                    dataset_batch["rgb_obs"], dataset_batch["depth_obs"], dataset_batch["robot_obs"]
                ) # perceptual_emb (batch, seq, 64), if static depth or gripper cam is not None, then (batch, seq, 128)
            else:
                perceptual_emb, perceptual_emb_seg = self.perceptual_encoder(
                    dataset_batch["rgb_obs"], dataset_batch["depth_obs"], dataset_batch["robot_obs"]
                ) # perceptual_emb_seg (batch, seq_num, 64), if static depth or gripper cam is not None, then (batch, seq_num, 128), seq_num = self.seg_num (10)
            ...
            if not self.use_temporal_contrastive_learning:
                ...
                kl, act_loss, mod_loss, pp_dist, pr_dist, seq_feat = self.lmp_seg_train(
